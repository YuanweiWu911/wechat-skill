...
- 微信消息后台驻守
- 闲聊自动回复
- 文件/图片/视频/语音附件接收（CDN 下载 + AES 解密，统一回复 `收到《文件名》，需要我帮忙吗？`）
- 安全请求自动执行
- 风险请求二次确认
- 项目内白名单文件发送
- 从 GUI 发送文本消息和文件
...
- `test-watcher.ts`
- `wechat-launcher.ts` — 一键启动器源码（启 watcher + 内嵌 HTTP 服务 + 开浏览器）
- `wechat-launcher.exe` — 编译后的独立可执行文件（双击即用，不依赖外部 Bun）
- `wechat-send.ts` — 命令行发送工具（文本/文件到微信用户）
...
1. 启动后台 watcher
2. 启动内置 GUI 服务器（已内嵌 `wechat-gui-server.ts` 全部 API）
...
编译产物为约 117MB 的单文件 exe。

### 核心文件

| 文件 | 说明 |
|------|------|
| `wechat-launcher.ts` | 一体化启动器源码（启 watcher + 内嵌 HTTP 服务器 + 开浏览器） |
...
- **Watcher 状态监控** — 顶部状态徽章实时显示运行/停止及 PID
- **启停控制** — 一键启动、停止、重启 watcher
- **消息发送** — 底部输入框输入文本，回车发送；📎 按钮选择文件发送
- **文件接收** — 附件自动下载（CDN + AES 解密），在气泡中显示原始文件名的可点击链接
- **会话浏览** — 左侧会话列表，按最后消息时间倒排，点击切换
- **用户昵称** — 点击会话列表头像设置显示昵称（localStorage 持久化）
- **对话气泡** — 类微信风格，用户消息居左、Bot 回复居右
- **防闪烁刷新** — 数据指纹比对后仅在实际变化时重建 DOM
- **历史关键词检索** — 模态框全局搜索
